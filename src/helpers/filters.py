...
def qs_vector_search(model: type[models.Model], query: Optional[str]) -> QuerySet:
...
def qs_filter(
    ...
    *,
    ...
    search_fields = search_fields or getattr(model, "SEARCH_FIELDS", ())

    assert search_fields is not None
    search_fields = (
        (*search_fields, "user__username")
        if "user__username" not in search_fields
        else search_fields
    )
    q = Q()
