...
from .models import Product, OrderProxy, Comment, Reply
...
class CommentAdmin(ModelAdmin):
    ...
    list_display = ("user__username", "message", "timestamp")
    list_filter = ("user",)
...
class ReplyAdmin(ModelAdmin):
    ...
    list_display = ("user__username", "message")


@admin.register(Product)
class ProductAdmin(
    GuardedModelAdminMixin, ImportExportMixin, ExportActionMixin, ModelAdmin
):
...
class OrderAdmin(ReadOnlyMixin, ExportMixin, ModelAdmin):
    ...
    search_fields = ("status", "user__username", "product__product_name")
    actions = (
        user_order_delivered_action,
        user_order_pending_action,
        user_order_cancelled_action,
